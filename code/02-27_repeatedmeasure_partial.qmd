...
Yield was measured at 4 time points, in days after planting (dap):  
  ...
  - 130  
  - 140  
  - 150  #if the last one was 160, the consecutive difference increases to 20 and would not work on this analysis
...
#crossing the band - significant, not crossed - non-significant
...
mod2_cs <- lme(yield_kgha ~ krate_kgha*nrate_kgha*fdap,
               ...
               data = rm_rcbd_df)  #numeric on correlation
...
#factor on model because we want mean,if numeric is given it'd do regression
#numeric on correlation to look at the intercept and slope or interval between them

#different interval dates, then, corCAR1 is used- continuous one and allows the different date interval
```
